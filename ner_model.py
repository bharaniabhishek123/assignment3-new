...
class NERModel(Model):
    ...
    def fit(self, sess, saver, train_examples_raw, dev_set_raw):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            # print len(train_examples) # 9166
            # A = minibatches(train_examples,batch_size=self.config.batch_size,shuffle=True)
            # B = next(A)
            # C = next(A)
            # print B
            # print len(B[0]) #2048
            ...
            # print len(C[0]) #2048
            # print len(C[1]) #2048
            for m in minibatches(train_examples,batch_size=self.config.batch_size,shuffle=True):
                # print m.shape
                # print l.shape
                n_minibatches += 1
                self.train_on_batch(sess, *m)

                #we can print but self.train_op will get updated everytime
