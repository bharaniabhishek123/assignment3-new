...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable("W_z",shape=(self.input_size,self.state_size),initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
            U_z = tf.get_variable("U_z",shape=(self.state_size,self.state_size),initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
            b_z = tf.get_variable("b_z",shape=(self.state_size),initializer=tf.constant_initializer(0),dtype=tf.float32)

            # inputs = tf.Print(inputs, [inputs],summarize=10,message='inputs')
            # W_z = tf.Print(W_z, [W_z],summarize=10,message='W_z')
            # state = tf.Print(state, [state],summarize=10,message='state')
            # U_z = tf.Print(U_z, [U_z],summarize=10,message='U_z')

            z_t = tf.nn.sigmoid(tf.matmul(inputs,W_z) + tf.matmul(state,U_z) + b_z)

            # z_t = tf.Print(z_t, [z_t],summarize=10,message='z_t')

            W_r = tf.get_variable("W_r",shape=(self.input_size,self.state_size),initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
            U_r = tf.get_variable("U_r",shape=(self.state_size,self.state_size),initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
            ...
            W_o = tf.get_variable("W_o", shape=(self.input_size, self.state_size),initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
            U_o = tf.get_variable("U_o", shape=(self.state_size, self.state_size),initializer=tf.contrib.layers.xavier_initializer(),dtype=tf.float32)
