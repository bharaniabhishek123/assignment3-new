...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self):
        ...
        ouputs,state = tf.nn.dynamic_rnn(cell,x,dtype=tf.float32)
        print state.shape
    ...
    def add_training_op(self, loss):
        ...
        gradient_and_variable = optimizer.compute_gradients(loss)

        gradients = [i[0] for i in gradient_and_variable]
        variables = [i[1] for i in gradient_and_variable]
        # - Remember to clip gradients only if self.config.clip_gradients
        # is True.
        # - Remember to set self.grad_norm
