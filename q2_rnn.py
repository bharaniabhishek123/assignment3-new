...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) < max_length :
            new_sen  = sentence + [zero_vector] * (max_length - len(sentence))
            new_lab  = labels + [zero_label] * (max_length - len(sentence))
            _masking = [True] * len(sentence) + [False] * (max_length - len(sentence))
            ret.append((new_sen,new_lab,_masking))

        elif len(sentence) > max_length:
            new_sen = sentence[:max_length]
            new_lab = labels[:max_length]
            _masking = [True] * max_length
            ret.append((new_sen,new_lab,_masking))
        else:
            _masking = [True] * len(sentence)
            ret.append((sentence,labels,_masking))
        pass
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embed = tf.get_variable("embedding_matrix",initializer=self.pretrained_embeddings)
        temp = tf.nn.embedding_lookup(embed,self.input_placeholder)

        # print temp.shape # output will be a 4-D tensor (?, 52, 6, 50)
        # Notice that shape is having max length here whereas in Neural Dependecy parser (HW2) we just had 2-D shape
        # We need to reshape temp into 3-D tensor

        embeddings = tf.reshape(temp ,(-1, self.max_length, self.config.n_features * self.config.embed_size))
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable("Weights_Layer2", shape=(self.config.hidden_size,self.config.n_classes)
                            ,initializer=tf.contrib.layers.xavier_initializer())
        b_2 = tf.get_variable("bias_Layer2",shape=(1,self.config.n_classes),initializer=tf.zeros_initializer)
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                # print o_t.shape
                # print h_0.shape
                tf.get_variable_scope().reuse_variables() # after calling cell for the first time setting reuse = true for variables
        ...
        preds = tf.stack(preds) # after this stack operation, preds will have shape like = (maxlength, None, n_classes)
    ...
    def add_loss_op(self, preds):
        ...
        out1 = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=mask_labels, logits=mask_preds)

        # out1 = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
